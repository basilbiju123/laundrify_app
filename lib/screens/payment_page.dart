...
class _PaymentPageState extends State<PaymentPage> {
  ...
  Widget _orderSummaryCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Order Summary',
            style: TextStyle(
                fontSize: 15, fontWeight: FontWeight.w800, color: t.textHi)),
        const SizedBox(height: 14),
        ...widget.services.map((svc) {
          ...
          return Padding(
              ...
              child: Row(children: [
                ...
                Expanded(
                    child: Text(svc['title'] ?? svc['serviceName'] ?? '',
        ...
        Container(
          ...
          decoration: BoxDecoration(
              color: AppColors.blue.withValues(alpha: t.isDark ? 0.15 : 0.06),
          ...
          child: Row(children: [
            ...
            Expanded(
                child: Text(
                    'Pickup: ${widget.pickupDate} at ${widget.pickupTime}',
                    style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Column(children: [
            Icon(icon,
                color:
                    active ? const Color(0xFF92700A) : const Color(0xFF94A3B8),
                size: 18),
  ...
  Widget _upiForm() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(
              controller: _upiCtrl,
              decoration:
                  _inputDeco('yourname@upi', Icons.alternate_email_rounded)),
          const SizedBox(height: 14),
          const Text('Popular UPI Apps',
              style: TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w600,
  ...
  Widget _cardForm() => Container(
        ...
        child: Column(children: [
          TextField(
              ...
              decoration:
                  _inputDeco('Cardholder Name', Icons.person_outline_rounded)),
          ...
          Row(children: [
            Expanded(
                child: TextField(
                    ...
                    keyboardType: TextInputType.number,
                    maxLength: 5,
                    decoration: _inputDeco('MM/YY', Icons.date_range_rounded,
                        counter: false))),
            ...
            Expanded(
                child: TextField(
                    ...
                    maxLength: 3,
                    obscureText: true,
                    decoration: _inputDeco('CVV', Icons.lock_outline_rounded,
                        counter: false))),
          ]),
        ]),
      );
  ...
  Widget _netBankingInfo() => Container(
        ...
        child: Column(children: [
          const Icon(Icons.account_balance_rounded,
              color: Color(0xFF1B4FD8), size: 40),
          const SizedBox(height: 12),
  ...
  Widget _codSection() => Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          const Text(
              'Pay with cash when your clothes are delivered back to you.',
              textAlign: TextAlign.center,
              style: TextStyle(
          ...
          Container(
            ...
            child: Row(children: const [
              ...
              Expanded(
                  child: Text(
                      'Exact change appreciated. Delivery agent carries limited change.',
                      style: TextStyle(
                          fontSize: 12,
                ...
                children: [
                  Container(
                      ...
                      child: const Icon(Icons.check_circle_rounded,
                          color: AppColors.emerald, size: 60)),
                  ...
                  Text(
                    ...
                    textAlign: TextAlign.center,
                    style:
                        TextStyle(fontSize: 15, color: t.textMid, height: 1.5),
                  ),
                  const SizedBox(height: 16),
                  if (_orderId != null)
                    Container(
  ...
  Future<void> _processPayment() async {
    ...
    if (_onlineOption == 'upi' && _upiCtrl.text.trim().isEmpty) {
      _snack('Please enter your UPI ID');
      return;
    }
    if (_onlineOption == 'card') {
      if (_cardNumberCtrl.text.replaceAll(' ', '').length < 16) {
        _snack('Please enter a valid 16-digit card number');
        return;
      }
      if (_cardExpiryCtrl.text.length < 5) {
        _snack('Please enter card expiry (MM/YY)');
        return;
      }
      if (_cardCvvCtrl.text.length < 3) {
        _snack('Please enter CVV');
        return;
      }
    }
  ...
  Future<void> _writeOrderToFirestore({
    ...
    try {
      ...
      try {
        ...
        final locData = userDoc.data()?['location'];
        if (locData is Map) {
          userAddress = (locData['address'] ?? locData['detectedAddress'] ?? '')
              .toString();
        }
      } catch (_) {}

      // Build flat items list for invoice/PDF
      ...
      for (final svc in widget.services) {
        for (final item in ((svc['items'] as List?) ?? [])) {
          ...
          if ((m['qty'] ?? 0) > 0) {
            items.add({
              'itemName': m['name'] ?? '',
              'serviceName': svc['title'] ?? svc['serviceName'] ?? '',
              'quantity': m['qty'] ?? 1,
              'price': m['price'] ?? 0,
            });
          }
      ...
      final docRef = await db.collection('orders').add({
        ...
        // Full services structure (for order cards & invoice)
        'services': widget.services,
        // Flat items list (for PDF invoice rows)
        'items': items,
        // Totals — all fields filled so orders page can always show them
        'subtotal': subtotal,
        ...
        'deliveryDate':
            widget.deliveryDate.isNotEmpty ? widget.deliveryDate : '',
        'deliveryTime':
            widget.deliveryTime.isNotEmpty ? widget.deliveryTime : '',
        // Order date for invoice
        'orderDate':
        ...
        'statusHistory': [
          {
            'status': 'pending',
            'note': 'Order placed via ${widget.paymentMethod.toUpperCase()}',
      ...
      try {
        ...
        await db
          ...
          'points': pts,
          'description':
              'Earned for order #${docRef.id.substring(0, 8).toUpperCase()}',
          'createdAt': FieldValue.serverTimestamp(),
        });
      } catch (loyaltyErr) {
