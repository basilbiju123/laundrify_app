  ...
  Future<void> _loadSettings() async {
    try {
      ...
      if (doc.exists && mounted) {
        final d = doc.data()!;
        setState(() {
          _commCtrl.text      = (d['commission'] ?? 10).toString();
          _minOrderCtrl.text  = (d['minOrderAmount'] ?? 99).toString();
          _maintenanceMode    = d['maintenanceMode'] ?? false;
          _phoneCtrl.text     = d['supportPhone']  ?? _phoneCtrl.text;
          _emailCtrl.text     = d['supportEmail']  ?? _emailCtrl.text;
          _hoursCtrl.text     = d['workingHours']  ?? _hoursCtrl.text;
          _facebookCtrl.text  = d['facebookUrl']   ?? _facebookCtrl.text;
          _instagramCtrl.text = d['instagramUrl']  ?? _instagramCtrl.text;
          _youtubeCtrl.text   = d['youtubeUrl']    ?? _youtubeCtrl.text;
        });
      }
    } catch (_) {}
  }
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _sectionHeader(Icons.info_outline_rounded, 'About System'),
                const SizedBox(height: 20),
                _infoRow('App Name', 'Laundrify'),
                _infoRow('Version', '2.0.0'),
                _infoRow('Backend', 'Firebase Firestore'),
                _infoRow('Auth', 'Firebase Authentication'),
                _infoRow('Built with', 'Flutter + Dart'),
              ],
            ),
          ),
        ],
