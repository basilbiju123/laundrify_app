...
class _AdminDashboardState extends State<AdminDashboard>
    ...
    try {
      ...
      final res = await Future.wait([
        ...
        _db
            .collection('users')
            .where('role',
                whereIn: ['delivery', 'manager', 'staff', 'employee'])
            .count()
            .get(),
        _db
            .collection('orders')
      ...
      double rev = 0, mRev = 0, dRev = 0;
      for (var d in (res[9] as QuerySnapshot).docs) {
        rev += ((d.data() as Map)['total'] ??
                (d.data() as Map)['totalAmount'] ??
                0)
            .toDouble();
      }
      for (var d in (res[10] as QuerySnapshot).docs) {
        mRev += ((d.data() as Map)['total'] ??
                (d.data() as Map)['totalAmount'] ??
                0)
            .toDouble();
      }
      for (var d in (res[11] as QuerySnapshot).docs) {
        dRev += ((d.data() as Map)['total'] ??
                (d.data() as Map)['totalAmount'] ??
                0)
            .toDouble();
      }
      final comp = (res[5] as AggregateQuerySnapshot).count ?? 1;

      if (mounted) {
        setState(() {
          totalUsers = (res[0] as AggregateQuerySnapshot).count ?? 0;
          totalOrders = (res[1] as AggregateQuerySnapshot).count ?? 0;
          activeOrders = (res[2] as AggregateQuerySnapshot).count ?? 0;
          totalEmployees = (res[3] as AggregateQuerySnapshot).count ?? 0;
          pendingOrders = (res[4] as AggregateQuerySnapshot).count ?? 0;
          completedOrders = comp;
          cancelledOrders = (res[6] as AggregateQuerySnapshot).count ?? 0;
          newUsersToday = (res[7] as AggregateQuerySnapshot).count ?? 0;
          newOrdersToday = (res[8] as AggregateQuerySnapshot).count ?? 0;
          totalRevenue = rev;
          monthRevenue = mRev;
          todayRevenue = dRev;
          avgOrderValue = comp > 0 ? rev / comp : 0;
        });
      }
    } catch (e) {
      debugPrint('Stats: $e');
  ...
  Future<void> _loadCharts() async {
    try {
      ...
      // Weekly
      final List<Map<String, dynamic>> wk = [];
      for (int i = 6; i >= 0; i--) {
        ...
        try {
          ...
          for (var d in snap.docs) {
            if (['completed', 'delivered'].contains((d.data())['status'])) {
              dayRev += ((d.data()['total'] ?? d.data()['totalAmount'] ?? 0))
                  .toDouble();
            }
          }
          wk.add({
          ...
          wk.add({'day': days[day.weekday - 1], 'count': 0.0, 'revenue': 0.0});
        }
      }

      // Status + service
      final all = await _db.collection('orders').get();
      ...
      for (int i = 5; i >= 0; i--) {
        ...
        try {
          final snap = await _db
              ...
              .where('createdAt', isLessThan: Timestamp.fromDate(mNext))
              .get();
          double mRev = 0;
          for (var d in snap.docs) {
            mRev += ((d.data()['total'] ?? d.data()['totalAmount'] ?? 0))
                .toDouble();
          }
          monList.add({
          ...
          monList
              .add({'month': months[m.month - 1], 'revenue': 0.0, 'orders': 0});
        }
      }
  ...
  Widget _sidebar() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(right: BorderSide(color: AdminTheme.cardBorder)),
      ),
      child: SafeArea(
          child: Column(children: [
          ...
          child: Row(children: [
            ...
            AnimatedBuilder(
              ...
              builder: (_, __) => Container(
                ...
                decoration: BoxDecoration(
                  color: AdminTheme.emerald.withValues(alpha: 0.12 + _pulseCtrl.value * 0.06),
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Container(width: 6, height: 6,
                    decoration: BoxDecoration(
                      color: AdminTheme.emerald.withValues(alpha: 0.5 + _pulseCtrl.value * 0.5),
                      shape: BoxShape.circle,
                      boxShadow: [BoxShadow(color: AdminTheme.emerald.withValues(alpha: 0.5), blurRadius: 4)],
                    )),
                  const SizedBox(width: 5),
                  const Text('LIVE', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, color: AdminTheme.emerald, letterSpacing: 1)),
                ]),
              ),
            ),
  ...
  Widget _home() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _welcome(),
        const SizedBox(height: 18),
        Text('Key Metrics', style: AdminTheme.heading(15)),
          ...
          child:
            ...
            Row(children: [
              Container(
                  ...
                  child: Icon(it.icon, color: it.color, size: 15)),
              const Spacer(),
              Container(
            ...
            Text(it.label, style: AdminTheme.label(10)),
          ]),
        );
      },
    );
  ...
  Widget _todayRow() {
    return Container(
      ...
      decoration: AdminTheme.cardDecoration(goldGlow: true),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
              ...
              child: const Icon(Icons.today_rounded,
                  color: AdminTheme.gold, size: 16)),
          const SizedBox(width: 9),
          Text("Today's Overview", style: AdminTheme.heading(14)),
          const Spacer(),
          Text(_fmtDate(DateTime.now()), style: AdminTheme.label(11)),
        ]),
        const SizedBox(height: 14),
        Row(children: [
  ...
  Widget _todayTile(String l, String v, Color c) => Expanded(
        child: Container(
          ...
          child: Column(children: [
            ...
            Text(l,
                style:
                    AdminTheme.label(9).copyWith(color: AdminTheme.textMuted),
  ...
  Widget _weeklyChart() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('Orders This Week', style: AdminTheme.heading(13)),
          const Spacer(),
          Text(
        ...
        SizedBox(
          ...
          child: weeklyOrders.isEmpty
              ? Center(child: Text('No data', style: AdminTheme.label(13)))
              : Row(
  ...
  Widget _statusChart() {
    ...
    return Container(
      ...
      decoration: AdminTheme.cardDecoration(),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Order Status', style: AdminTheme.heading(13)),
        const SizedBox(height: 3),
        Text('$total total orders', style: AdminTheme.label(10)),
        const SizedBox(height: 12),
        if (total == 0)
          Center(child: Text('No orders yet', style: AdminTheme.label(13)))
        else
          ...orderStatus.entries.take(7).map((e) {
            final pct = e.value / total;
            final c = colors[e.key] ?? AdminTheme.textSecondary;
            return Padding(
  ...
  Widget _revenueChart() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('6-Month Revenue', style: AdminTheme.heading(13)),
          const Spacer(),
          Container(
        ...
        SizedBox(
          ...
          child: monthlyRevenue.isEmpty
              ? Center(
                  child: Text('No revenue data', style: AdminTheme.label(13)))
              : Row(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: monthlyRevenue.asMap().entries.map((entry) {
                    ...
                    return Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 5),
                          Text(e['month'], style: AdminTheme.label(9)),
                          Text('${e['orders']}',
                              style: const TextStyle(
                                  fontSize: 8,
  ...
  Widget _liveOrders() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('Live Orders', style: AdminTheme.heading(13)),
          const Spacer(),
          AnimatedBuilder(
              animation: _pulseCtrl,
              builder: (_, __) =>
            ...
            if (!snap.hasData || snap.data!.docs.isEmpty) {
              return Padding(
                  ...
                  child: Center(
                    ...
                    Text('No active orders', style: AdminTheme.label(12)),
                  ])));
            }
            return Column(
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    ...
                    if (ts != null) ...[
                      ...
                      Text('${ts.hour}:${ts.minute.toString().padLeft(2, '0')}',
                          style: AdminTheme.label(8)
                              .copyWith(color: AdminTheme.textMuted))
                    ],
                  ]),
                ]),
              );
  ...
  Widget _serviceCard() {
    ...
    return Container(
      ...
      decoration: AdminTheme.cardDecoration(),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Services', style: AdminTheme.heading(13)),
        const SizedBox(height: 3),
        Text('$total orders across ${serviceTypes.length} types',
            style: AdminTheme.label(10)),
        const SizedBox(height: 12),
        if (serviceTypes.isEmpty)
          Center(child: Text('No data', style: AdminTheme.label(13)))
        else
          ...serviceTypes.entries.map((e) {
            final c = sColors[e.key] ?? AdminTheme.textSecondary;
  ...
  Widget _recentUsers() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('Recent Signups', style: AdminTheme.heading(13)),
          const Spacer(),
          GestureDetector(
        ...
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (_, snap) {
            ...
            if (snap.data!.docs.isEmpty) {
              return Center(
                  child: Text('No users yet', style: AdminTheme.label(13)));
            }
            return Column(
                children: snap.data!.docs.map((doc) {
              final d = doc.data() as Map<String, dynamic>;
              ...
              return ListTile(
                ...
                title: Text(name,
                    style: const TextStyle(
                        ...
                        fontWeight: FontWeight.w800,
                        color: AdminTheme.textPrimary)),
                subtitle: Text(d['email'] ?? '', style: AdminTheme.label(10)),
                trailing: ts != null
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('App Usage Summary', style: AdminTheme.heading(14)),
        ]),
        const SizedBox(height: 14),
        StreamBuilder<QuerySnapshot>(
  ...
  Widget _usageTile(String l, String v, Color c, IconData icon) => Expanded(
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 12),
          decoration: BoxDecoration(
              color: c.withValues(alpha: 0.07),
          ...
          child: Column(children: [
            ...
            Text(l,
                style:
                    AdminTheme.label(8).copyWith(color: AdminTheme.textMuted),
                textAlign: TextAlign.center),
          ]),
        ),
      );
